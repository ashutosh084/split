...
// .wrangler/tmp/bundle-dbbj8U/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-dbbj8U/middleware-loader.entry.ts
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
// .wrangler/tmp/bundle-dbbj8U/middleware-insertion-facade.js
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
friendRoutes.post("/", zValidator("json", addFriendSchema), async (c) => {
  ...
  const currentUser = c.get("user");
  const db = c.env.DB;
  const friend = await db.prepare("SELECT id, email, name FROM Users WHERE email = ?").bind(friendEmail).first();
  if (!friend) {
    return c.json({ error: "User not found" }, 404);
  }
  ...
  ).bind(currentUser.userId, friend.id, friend.id, currentUser.userId).first();
  if (existing) {
    return c.json({ error: "Already friends" }, 409);
  }
  await db.prepare("INSERT INTO Friends (user_id_1, user_id_2) VALUES (?, ?)").bind(currentUser.userId, friend.id).run();
  return c.json(
    {
