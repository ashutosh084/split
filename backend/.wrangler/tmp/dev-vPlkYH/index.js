...
// .wrangler/tmp/bundle-3mfiTK/strip-cf-connecting-ip-header.js
...
var init_check_key_length = __esm({
  "node_modules/jose/dist/browser/runtime/check_key_length.js"() {
    ...
    init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
    init_performance2();
    check_key_length_default = /* @__PURE__ */ __name((alg2, key) => {
...
var init_normalize_key = __esm({
  "node_modules/jose/dist/browser/runtime/normalize_key.js"() {
    ...
    importAndCache = /* @__PURE__ */ __name(async (cache, key, jwk, alg2, freeze = false) => {
    ...
    normalizePublicKey = /* @__PURE__ */ __name((key, alg2) => {
      if (isKeyObject(key)) {
    ...
    normalizePrivateKey = /* @__PURE__ */ __name((key, alg2) => {
      if (isKeyObject(key)) {
        let jwk = key.export({ format: "jwk" });
        if (jwk.k) {
          return exportKeyValue(jwk.k);
...
var init_check_key_type = __esm({
  "node_modules/jose/dist/browser/lib/check_key_type.js"() {
    ...
    jwkMatchesOp = /* @__PURE__ */ __name((alg2, key, usage) => {
    ...
    symmetricTypeCheck = /* @__PURE__ */ __name((alg2, key, usage, allowJwk) => {
    ...
    asymmetricTypeCheck = /* @__PURE__ */ __name((alg2, key, usage, allowJwk) => {
      if (allowJwk && isJWK(key)) {
        switch (usage) {
          case "sign":
...
var init_verify = __esm({
  "node_modules/jose/dist/browser/runtime/verify.js"() {
    ...
    verify = /* @__PURE__ */ __name(async (alg2, key, signature, data) => {
...
var init_sign = __esm({
  "node_modules/jose/dist/browser/runtime/sign.js"() {
    ...
    init_check_key_length();
    init_get_sign_verify_key();
    sign = /* @__PURE__ */ __name(async (alg2, key, data) => {
...
async function createJwt(payload, secret) {
  ...
  return new SignJWT({ ...payload }).setProtectedHeader({ alg }).setIssuedAt().setExpirationTime("24h").sign(encoder2.encode(secret));
}
...
function extractToken(c) {
  ...
  if (cookie) {
    const match2 = cookie.match(/(?:^|;\s*)auth_token=([^;]*)/);
    if (match2)
      return match2[1];
  }
  return null;
}
async function getAuthPayload(c, secret) {
...
// .wrangler/tmp/bundle-3mfiTK/middleware-loader.entry.ts
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
// .wrangler/tmp/bundle-3mfiTK/middleware-insertion-facade.js
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
authRoutes.post("/register", zValidator("json", registerSchema), async (c) => {
  ...
  return c.json(
    {
      message: isFirstUser ? "Admin account created. You may now log in." : "Registration successful. Await admin approval."
    },
    201
  );
});
authRoutes.post("/login", zValidator("json", loginSchema), async (c) => {
  ...
  return c.json({
    ...
    token
    // also return token for Authorization header usage
  });
});
...
authRoutes.get("/me", async (c) => {
  const { getAuthPayload: getAuthPayload2 } = await Promise.resolve().then(() => (init_jwt(), jwt_exports));
  const payload = await getAuthPayload2(c, c.env.JWT_SECRET);
  if (!payload) {
    return c.json({ error: "Unauthorized" }, 401);
  }
  const db = c.env.DB;
  const user = await db.prepare("SELECT id, email, name, is_admin FROM Users WHERE id = ?").bind(payload.userId).first();
  if (!user) {
...
init_performance2();
init_jwt();
var authRequired = /* @__PURE__ */ __name(async (c, next) => {
...
var adminRequired = /* @__PURE__ */ __name(async (c, next) => {
...
adminRoutes.patch("/users/:id/approve", async (c) => {
  ...
  const db = c.env.DB;
  const user = await db.prepare("SELECT id, is_approved FROM Users WHERE id = ?").bind(userId).first();
  if (!user) {
    return c.json({ error: "User not found" }, 404);
  }
  if (user.is_approved) {
...
adminRoutes.delete("/users/:id", async (c) => {
  ...
  const user = await db.prepare("SELECT id, is_admin FROM Users WHERE id = ?").bind(userId).first();
  if (!user) {
    return c.json({ error: "User not found" }, 404);
  }
  const debtCheck = await db.prepare(
  ...
  await db.batch([
    ...
    db.prepare("DELETE FROM Friends WHERE user_id_1 = ? OR user_id_2 = ?").bind(userId, userId),
...
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var addFriendSchema = external_exports.object({
  friendEmail: external_exports.string().email()
});
var friendRoutes = new Hono2();
friendRoutes.use("*", authRequired);
friendRoutes.post("/", zValidator("json", addFriendSchema), async (c) => {
  const { friendEmail } = c.req.valid("json");
  const currentUser = c.get("user");
  const db = c.env.DB;
  const target = await db.prepare("SELECT id, email, name FROM Users WHERE email = ?").bind(friendEmail).first();
  if (!target) {
    return c.json({ error: "User not found" }, 404);
  }
  ...
  ).bind(currentUser.userId, target.id, target.id, currentUser.userId).first();
  ...
  if (existingRequest) {
    if (existingRequest.status === "pending") {
      ...
      await db.prepare("DELETE FROM FriendRequests WHERE id = ?").bind(existingRequest.id).run();
      await db.prepare("INSERT INTO Friends (user_id_1, user_id_2) VALUES (?, ?)").bind(currentUser.userId, target.id).run();
      return c.json(
        {
  ...
  ).bind(requestId, currentUser.userId, target.id, now()).run();
  return c.json(
    {
...
friendRoutes.get("/", async (c) => {
  ...
  ).bind(currentUser.userId, currentUser.userId, currentUser.userId).all();
...
friendRoutes.post("/requests/:id/accept", async (c) => {
  ...
  const currentUser = c.get("user");
  const db = c.env.DB;
  const req = await db.prepare("SELECT * FROM FriendRequests WHERE id = ?").bind(requestId).first();
  if (!req) {
  ...
  await db.prepare("INSERT INTO Friends (user_id_1, user_id_2) VALUES (?, ?)").bind(req.from_user_id, req.to_user_id).run();
  await db.prepare("UPDATE FriendRequests SET status = 'accepted' WHERE id = ?").bind(requestId).run();
...
friendRoutes.post("/requests/:id/reject", async (c) => {
  ...
  await db.prepare("UPDATE FriendRequests SET status = 'rejected' WHERE id = ?").bind(requestId).run();
...
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var splitSchema = external_exports.object({
  userId: external_exports.string().uuid(),
  amountOwed: external_exports.number().positive()
});
var createExpenseSchema = external_exports.object({
  amount: external_exports.number().positive(),
  name: external_exports.string().min(1).max(200),
  description: external_exports.string().max(500).optional(),
  tags: external_exports.array(external_exports.string().min(1).max(50)).optional(),
  splits: external_exports.array(splitSchema).min(1)
});
...
expenseRoutes.post("/", zValidator("json", createExpenseSchema), async (c) => {
  ...
  if (tags && tags.length > 0) {
    for (const tagName of tags) {
      let tag2 = await db.prepare("SELECT id FROM Tags WHERE name = ?").bind(tagName).first();
      if (!tag2) {
        const tagId = uuid();
      ...
      statements.push(
        db.prepare("INSERT INTO ExpenseTags (expense_id, tag_id) VALUES (?, ?)").bind(expenseId, tag2.id)
      );
      await db.prepare(
        "INSERT OR IGNORE INTO UserTags (user_id, tag_id) VALUES (?, ?)"
      ).bind(currentUser.userId, tag2.id).run();
    }
  }
...
expenseRoutes.patch("/:id/settle", async (c) => {
  ...
  const currentUser = c.get("user");
  const db = c.env.DB;
  const expense = await db.prepare("SELECT id, payer_id FROM Expenses WHERE id = ?").bind(expenseId).first();
  if (!expense) {
    return c.json({ error: "Expense not found" }, 404);
  }
  const split = await db.prepare(
    "SELECT id, user_id, is_paid FROM ExpenseSplits WHERE expense_id = ? AND user_id = ?"
  ).bind(expenseId, currentUser.userId).first();
  if (!split) {
  ...
  ).bind(expenseId, currentUser.userId).run();
...
userRoutes.get("/me/tags", async (c) => {
  ...
  const db = c.env.DB;
  const userId = currentUser.userId;
  const tagBreakdown = await db.prepare(
